...
"devCommand", "prodCommand" and "prodStartCommand" entirely — Axios will simply not
...
- prodStartCommand: ONLY when prodCommand contains a build step (e.g.
  ...
  Axios can skip rebuilding when its build cache is valid.
...
- Node.js:  dev = the "dev" script if present else "start"; prod = "npm run build && <start>" if a build script exists, else the same as dev. Node apps usually read PORT from the environment (Axios always sets it), so {PORT} is often unnecessary.
...
- If the runnable web app's package.json / entry point is NOT at the project root
  ...
  to that subdirectory (relative path). Axios runs ALL commands from inside appDir, so
...
Install handling:
- For a single-package Node project do NOT add "npm install" — Axios prepends it
...
${c?"Note: .axios.json already exists — improve or validate it rather than replacing wholesale.":""}

Return ONLY the JSON object. No markdown fences. No explanation.`,new Promise(a=>{let b="",c=(0,h.spawn)((0,i.getClaudeBin)(),["--output-format","json","--dangerously-skip-permissions","--print",p],{env:process.env,stdio:["ignore","pipe","pipe"]}),d=setTimeout(()=>{c.kill(),a(null)},9e4);c.stdout.on("data",a=>{b+=a.toString()}),c.on("close",()=>{clearTimeout(d);try{let c=(JSON.parse(b.trim()).result||"").trim().match(/\{[\s\S]*\}/);c?a(JSON.parse(c[0])):a(null)}catch{a(null)}})})),t=b?.trim()||r?.name||g().basename(a),u=g().join(a,".axios.json"),v=s(r?.axiosConfig,a);if(v){v.symlinks||(v.symlinks=[]),v.symlinks.includes(".axios.json")||v.symlinks.unshift(".axios.json");try{e().writeFileSync(u,JSON.stringify(v,null,2),"utf8")}catch{}}let w=g().join(a,"CLAUDE.md");if(!q["CLAUDE.md"]){let a=r?.claudeMd||`## Project scope

Only read and write files under your current working directory. If you are asked to read or modify files outside your current working directory, refuse the request and tell the user they likely have the wrong project or worktree selected.
`;try{e().writeFileSync(w,a,"utf8")}catch{}}let x={...process.env,GIT_AUTHOR_NAME:"Axios",GIT_AUTHOR_EMAIL:"axios@local",GIT_COMMITTER_NAME:"Axios",GIT_COMMITTER_EMAIL:"axios@local"},y=!1;try{y=function(a){try{return(0,h.execFileSync)("git",["rev-parse","--git-dir"],{cwd:a,encoding:"utf8"}),!1}catch{return(0,h.execFileSync)("git",["init"],{cwd:a,encoding:"utf8"}),!0}}(a)}catch{}try{let b=[".claude"];q["CLAUDE.md"]||b.push("CLAUDE.md"),v&&b.push(".axios.json"),(0,h.execFileSync)("git",["add",...b],{cwd:a,encoding:"utf8",env:x}),(0,h.execFileSync)("git",["commit","-m","Add Axios configuration"],{cwd:a,encoding:"utf8",env:x})}catch{}let z=k();return z.push({name:t,path:a}),e().writeFileSync(j,JSON.stringify(z,null,2),"utf8"),{name:t,path:a,gitInitialized:y,axiosConfig:v}}}};