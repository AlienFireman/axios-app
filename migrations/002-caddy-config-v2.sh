#!/usr/bin/env bash
# Migration 002 — bring private.Caddyfile up to canonical "config v2".
#
# Why: the device-session cookie name became per-install (`termato_auth_<slug>` instead
# of the fixed `termato_auth` — see app/lib/requestMeta.cjs cookieName()). The preview
# blocks strip that cookie before proxying to the previewed project so it never receives
# the user's session token; the v1 strip pattern matched only the literal `termato_auth=`
# and would MISS `termato_auth_dan=…`, leaking the token into the preview backend. v2
# widens the strip to `termato_auth[A-Za-z0-9_]*=`. Also re-applies the `forward_auth`
# session gate for any install whose Caddyfile predates it (same as v1).
#
...
# add a new migration (003-…) when those rules change.

set -uo pipefail

CADDY_CONFIG_VERSION=2
