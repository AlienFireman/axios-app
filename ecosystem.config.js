...
module.exports = {
  apps: [{
    ...
    script: 'server.js',
    args: '',
    // Resolve to wherever Axios is installed (e.g. ~/.axios). pm2 sets the app's
    // working dir to this, which is what every data/projects path derives from
    // via process.cwd().
    cwd: __dirname,
    // Run under the Node the installer bundled into ~/.axios/.node, so the runtime
    // matches the build regardless of the user's system Node. Unset (e.g. in dev on
    // the hub) → pm2 uses its default node.
    interpreter: process.env.AXIOS_NODE_BIN || undefined,
    env: {
      ...
      // server.js runs `next dev` only when AXIOS_DEV==='1'; pinning '0' forces
      // prod and overrides any stray AXIOS_DEV inherited from the pm2 daemon env.
      NODE_ENV: 'production',
      AXIOS_DEV: '0',
      PORT: String(port),
    },
